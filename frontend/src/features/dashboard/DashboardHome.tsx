                        ...
                        <div className="lg:col-span-7">
                            ...
                            <div className="bg-gradient-to-br from-primary-600/20 to-success-600/10 glass-card p-8 shadow-2xl transform hover:scale-[1.01] transition-all duration-300 border-primary-500/30">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="bg-dark-card/60 rounded-xl p-4 backdrop-blur-sm border border-dark-border/50">
                                        <p className="text-dark-text-tertiary text-xs mb-1">{t('dashboard.pnl24h')}</p>
                                        <p className="text-dark-text-primary text-xl font-bold">{formatCurrency(247.83 * (currencyState.rates['USD'] || 1), currencyState)}</p>
                                        <p className="text-success-400 text-sm">+2.3%</p>
                                    </div>
                                     <div className="bg-dark-card/60 rounded-xl p-4 backdrop-blur-sm border border-dark-border/50">
                                        <p className="text-dark-text-tertiary text-xs mb-1">{t('dashboard.assets')}</p>
                                        <p className="text-dark-text-primary text-xl font-bold">{cryptoList?.length || 0}</p>
                                        <p className="text-dark-text-secondary text-sm">{t('dashboard.cryptocurrencies')}</p>
                                    </div>
                                     <div className="bg-dark-card/60 rounded-xl p-4 backdrop-blur-sm border border-dark-border/50">
                                        <p className="text-dark-text-tertiary text-xs mb-1">{t('dashboard.income')}</p>
                                        <p className="text-dark-text-primary text-xl font-bold">{formatCurrency(1834 * (currencyState.rates['USD'] || 1), currencyState)}</p>
                                        <p className="text-primary-400 text-sm">{t('dashboard.monthly')}</p>
                                    </div>
                                </div>
                             ...
                             <div className="glass-card p-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-sm font-semibold text-success-400">73 ({t('dashboard.greed')})</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                {topGainers.map((crypto, index) => (
                                    <div key={crypto.id} className="flex items-center justify-between p-4 bg-dark-card/50 rounded-xl hover:bg-dark-hover/50 transition-colors cursor-pointer border border-dark-border/50" onClick={() => navigate('/trading')}>
                                        <div className="flex items-center gap-4"><span className="text-dark-text-tertiary font-mono text-sm w-6">{index + 1}</span><img src={crypto.image} alt={crypto.symbol} className="w-10 h-10 rounded-full" /><div><p className="font-semibold">{crypto.symbol.toUpperCase()}</p><p className="text-xs text-dark-text-tertiary">{crypto.name}</p></div></div>
                                        <div className="text-right"><p className="font-mono font-semibold">{formatCurrency(crypto.price, currencyState)}</p><div className="flex items-center justify-end gap-1 text-success-400 text-sm"><ArrowUpRight className="w-4 h-4" />+{crypto.change_percent_24h.toFixed(2)}%</div></div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="glass-card p-6">
                            ...
                            <div className="space-y-3">
                                {topLosers.map((crypto, index) => (
                                     <div key={crypto.id} className="flex items-center justify-between p-4 bg-dark-card/50 rounded-xl hover:bg-dark-hover/50 transition-colors cursor-pointer border border-dark-border/50" onClick={() => navigate('/trading')}>
                                        <div className="flex items-center gap-4"><span className="text-dark-text-tertiary font-mono text-sm w-6">{index + 1}</span><img src={crypto.image} alt={crypto.symbol} className="w-10 h-10 rounded-full" /><div><p className="font-semibold">{crypto.symbol.toUpperCase()}</p><p className="text-xs text-dark-text-tertiary">{crypto.name}</p></div></div>
                                        <div className="text-right"><p className="font-mono font-semibold">{formatCurrency(crypto.price, currencyState)}</p><div className="flex items-center justify-end gap-1 text-danger-400 text-sm"><ArrowDownRight className="w-4 h-4" />{crypto.change_percent_24h.toFixed(2)}%</div></div>
                                    </div>
                                ))}
                            </div>
                        </div>
